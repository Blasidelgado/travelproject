 ...
 */
export default async function myJourneys(navigateTo, page=1) {
    ...
    if (response.success) {
        ...
        if (info.length > 0) {
            ...
            prevBtn.onclick = async () => await navigateTo('/my-journeys', parseInt(page) - 1);
            nextBtn.onclick = async () => await navigateTo('/my-journeys', parseInt(page) + 1);
